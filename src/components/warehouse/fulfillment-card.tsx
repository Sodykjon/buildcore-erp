...
type FulfillmentStatus = {
  ...
  customer:    string
  items: {
    orderItemId:       string  // <-- added for fulfillment
    product:           string
    unit:              string
...
export function FulfillmentCard({ orderId }: { orderId: string }) {
  ...
  if (done) {
    return (
      <div className="bg-gray-900 rounded-xl border border-green-800/50 p-6 flex items-center gap-3">
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 p-5 space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <p className="font-semibold text-white font-mono">{order.orderNumber}</p>
        ...
        {order.items.map(item => (
          <div key={item.orderItemId} className="space-y-1.5">
            ...
            {item.quantityRemaining > 0 && (
              <div className="flex items-center gap-2">
                <label className="text-xs text-gray-500">Pick up now:</label>
                <input
                  type="number"
                  min={0}
                  ...
                  className="w-20 bg-gray-800 border border-gray-700 rounded-lg px-2 py-1
                             text-sm font-mono text-white outline-none focus:border-amber-500
      ...
      <button
        ...
        className="w-full py-2.5 rounded-lg bg-amber-500 hover:bg-amber-400 text-gray-950
                   font-semibold text-sm transition-all disabled:opacity-40 flex items-center
